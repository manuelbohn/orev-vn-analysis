...
```{r}
verbs <- read_xlsx("../data/verbs_overview.xlsx")%>%
...
determine_size_v%>%
  ...
  pivot_longer(cols = c(ratio), names_to = "type",values_to = "value")%>% 
  ggplot(aes(x = factor(size), y = value))+
  geom_hline(yintercept = 1, lty = 3, alpha =0.75)+
  geom_point(pch = 1, size = 3)+
  #geom_line()+
  #facet_grid(type~. , scales = "free_y")+
  theme_bw()+
  labs(x = "No. of items", y = "Model comparison ratio\nabs(elpd_diff)/(2*se_diff)")
...
sel_verbs%>%
  ...
  group_by(items)%>%
  summarise(sum = n())%>%
  ggplot(aes(x = reorder(items, -sum), y = sum/50))+
...
psize <- determine_size_v%>%
  ...
  #facet_grid(type~. , scales = "free_y")+
  theme_bw()+
  labs(x = "No. of items", y = expression(paste("Model comparison: ", frac(Delta ~ elpd, "2 *"~SE(Delta ~ elpd)))))
```
